<script lang="ts">
	...
	export let isOpen: boolean = $favoriteSheetOpen;
	// export let position: string = 'left';

	$: {
		isOpen = $favoriteSheetOpen;
		console.log('favorite items', $favoriteItems);
	}

	if (browser) {
		handleEscClose($favoriteSheetOpen);
	}
</script>
...
>
	<div class="flex h-screen flex-col border bg-white shadow-md">
		...
		<div class="flex items-center justify-between border-b p-2">
			<p class="font-normal text-xl ml-2">Favorites</p>
			<div class="flex">
				<button
					on:click={() => ($favoriteSheetOpen = !$favoriteSheetOpen)}
		...
		<!-- Content -->
		{#if $favoriteItemsCount === 0 || undefined}
			<div class="flex items-center justify-center text-center flex-col max-w-xs mx-auto">
				...
				<p class="text-base text-slate-600 mb-4">
					No items in your wishlist, go to shop and to add some
				</p>
